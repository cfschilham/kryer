...
// for passwords because it spawns a new goroutine for each password in pwd, and spawning, for
// instance, 1000 goroutines for 1000 `*loadcfg.Dict.Pwds()` entries is not going to work well.
//
...
func SSHDictMT(host loadcfg.Host, config *loadcfg.Config, pwds []string) (string, error) {
	...
	// A waitgroup is created. If `<-SendChanOnWGWait(&wg)` receives a value, all goroutines have
